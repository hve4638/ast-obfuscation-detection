...
obfuscated_normal_dir_1 = "D:/PowerShell/1119/extractedmatrix/extracted(imp)/IseSteroids/npy"
obfuscated_normal_dir_2 = "D:/PowerShell/1119/extractedmatrix/extracted(imp)/invokeObfuscation/npy"
obfuscated_normal_dir_3 = "D:/PowerShell/1119/extractedmatrix/extracted(imp)/IseSteroids/npy"
attack_dir_2 = "D:/PowerShell/1119/extractedmatrix/extracted(imp)/EncodedPowershell(obfuscated)/npy"

# 검증 데이터 경로 설정
validation_dir = "D:/PowerShell/unknownvalidationset/extracted(imp)/rc_powershell/npy"
...
# 검증 데이터 불러오기: Unknown 검증 데이터셋 (악성 난독화로 가정하여 라벨 1 설정)
validation_data = load_data_from_npy(validation_dir, label=1)

# 검증 데이터셋의 20%를 학습 데이터로 사용하고 나머지는 검증에 사용
num_validation_samples = len(validation_data)
validation_indices = np.random.choice(num_validation_samples, size=int(num_validation_samples * 0.2), replace=False)

# 학습에 사용할 검증 데이터셋 샘플과 검증에 사용할 나머지 데이터셋 분리
train_from_validation = [validation_data[i] for i in validation_indices]
validation_remaining = [validation_data[i] for i in range(num_validation_samples) if i not in validation_indices]

# 학습 데이터에 검증 데이터셋의 일부 추가
all_data += train_from_validation

# 검증 데이터셋을 갱신
validation_data = validation_remaining
