...
def train_cnn_densenet_model():
    ...
    # 검증 데이터 로드 및 정규화
    validation_images, validation_labels = load_images_from_folder(validation_dir, img_size, label=1)  # unknown은 악성 난독화로 가정
    X_val = validation_images / 255.0

    # 학습 데이터에 unknown 검증 데이터셋의 20% 추가
    val_indices = np.random.choice(len(X_val), size=int(len(X_val) * 0.2), replace=False)
    X_train = np.concatenate((X_train, X_val[val_indices]), axis=0)
    y_train = np.concatenate((y_train, np.ones(len(val_indices))), axis=0)

    # 나머지 80%는 검증 데이터셋으로 유지
    X_val = np.delete(X_val, val_indices, axis=0)
    y_val = np.delete(validation_labels, val_indices, axis=0)
