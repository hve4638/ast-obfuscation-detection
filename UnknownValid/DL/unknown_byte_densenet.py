...
def train_cnn_densenet_model():
    ...
    # 검증 데이터 로드 및 정규화
    validation_images, validation_labels = load_images_from_folder(validation_dir, img_size, label=-1)
    X_val = validation_images / 255.0
    y_val = np.array([0 if i < len(validation_images) / 2 else 1 for i in range(len(validation_images))])
