...
# 3-gram AST 파일들을 읽어들여 리스트로 반환하는 함수
def read_3gram_files(directory):
    ngram_texts = []
...
# 디렉토리로부터 3-gram 파일들을 읽어들이는 함수
def gather_data_from_directories(directories, label):
    all_texts = []
    all_labels = []
    for directory in directories:
        ngram_texts = read_3gram_files(directory)
        all_texts.extend(ngram_texts)
        all_labels.extend([label] * len(ngram_texts))
    return all_texts, all_labels

# 경로 설정
benign_ast_dir = 'D:/PowerShell/Extraction/GithubGist/GithubGist(3gram)'

obfuscated_benign_ast_dirs = [
    'D:/PowerShell/Extraction/InvokeCradleCrafter/InvokeCradleCrafter(3gram)',
    'D:/PowerShell/Extraction/InvokeObfuscation/InvokeObfuscation(3gram)',
    'D:/PowerShell/Extraction/IseSteroids/IseSteroids(3gram)'
]

malicious_ast_dir = 'D:/PowerShell/Extraction/mentor_dataset/decoded/decoded(3gram)'

obfuscated_malicious_ast_dir = 'D:/PowerShell/Extraction/mentor_dataset/encoded/mentor(3gram)'
...
# TF-IDF 벡터화 (3-gram 데이터를 처리)
vectorizer = TfidfVectorizer(ngram_range=(3, 3))  # 3-gram 기반으로 벡터 변환
