...
def train_fcdnn_model():
    ...
    # 데이터 결합
    X = np.concatenate((obfuscated_normal_images_1, obfuscated_normal_images_2, obfuscated_normal_images_3, attack_images_2), axis=0)
    y = np.concatenate((obfuscated_normal_labels_1, obfuscated_normal_labels_2, obfuscated_normal_labels_3, attack_labels_2), axis=0)
    ...
    # FC-DNN 모델 구성
    model = Sequential([
        Input(shape=(img_size[0] * img_size[1] * 3,)),  # Flattened input shape
    ...
    # 모델 학습
    history = model.fit(X_train_flat, y_train, epochs=epochs, validation_data=(X_test_flat, y_test))
